...
# --- Evaluate Model 1: Baseline (TF-IDF) ---
...
# --- Evaluate Model 2: Main Multimodal (PyTorch) ---
...
try:
    ...
    # --- This block loads the model defined in the config ---
    main_model_arch = config.IMAGE_MODEL_NAME 
    main_model_path = config.MODEL_SAVE_PATH
    
    if not os.path.exists(main_model_path):
         # Try the other path just in case
         main_model_path = config.HYBRID_MODELS_FOR_FEATURES.get(main_model_arch)
         if not os.path.exists(main_model_path):
...
# --- Evaluate Model 3: Strong Baseline (DistilBERT + IPQ) ---
...
try:
    ...
    if test_ipq.ndim == 1: test_ipq = test_ipq.reshape(-1, 1)
    
    model_arch = list(config.HYBRID_MODELS_FOR_FEATURES.keys())[0] # Get arch name
...
# --- Evaluate Model 4: FINAL Hybrid (DistilBERT + Image + IPQ) ---
...
# --- Display Results ---
print("\n\nFinal Evaluation Results")
results_df = pd.DataFrame(results).T.sort_values(by='SMAPE') # Sort by SMAPE
print(results_df.to_string())
print("----------------------------------")
